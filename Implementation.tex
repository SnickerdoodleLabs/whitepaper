...
\begin{algorithm}
\caption{Key Ratchet Pseudo-code}
\label{alg:KeyRatchet}
    \begin{algorithmic}
        \Require EOA with message signing utility
        \Ensure New EOA with no prior transaction history
        \State Entropy $\gets$ EOA.signMessage(Pre-determined Message)
        \State new EOA $\gets$ HMAC-SHA256(Entropy)
        \State return new EOA
    \end{algorithmic}
\end{algorithm}

Procedure \ref{alg:KeyRatchet} outlines the a simple technique to generate a new key from the message signing utility exposed from most consumer crypto-wallets. This 
...
\subsection{Aggregation Services} % name for role?
